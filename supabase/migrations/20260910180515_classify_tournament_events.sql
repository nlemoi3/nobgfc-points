...
add column if not exists is_tournament boolean not null default true;

update public.events
set is_tournament = false
where id = 1
  and name = 'Spring Awards Banquet - Louisiana Champions Cup Presentation';
...
begin
  ...
  if validate_assignment then
    if not selected_event.is_tournament then
      raise exception 'Catches can only be assigned to tournament events.'
        using errcode = '23514';
    end if;
